...
- [3. How to use](#3-how-to-use)
    ...
    - [3.2. Use-case for non-root websites (like `domain.com/website/`)](#32-use-case-for-non-root-websites-like-domaincomwebsite)
...
- [6. Changes in version `0.6.0`](#6-changes-in-version-060)
- [7. Licence](#7-licence)
...
the landing page of the repository thus it is renamed to `index.html`.

### 2.2. Specifics

#### 2.2.1. Markdown format

The [default Pandoc script](pandoc.sh) converts from `markdown` to `html` with following
...
As [mentioned above](#211-markdown-format) the Markdown format we’re using
...
Use it as a part of you GH Actions workflow:
...
where you can put your Markdown documents to convert alongside other files,
that will be ignored.

It is advised you should look at some use-cases listed below,
...
### 3.2. Use-case for non-root websites (like `domain.com/website/`)

When dealing with a website that will not be anchored at the root of the domain,
you need to specify the website root.

For example say, you have a repository on GitHub you want to publish.
...
This is useful if the website is anchored not on the root of the domain.
...
If not provided, the [template contained in this repository](template/head.html) will be used.
...
This options may be handy in a use-case where you want to publish
